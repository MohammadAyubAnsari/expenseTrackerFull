...
form.addEventListener("submit", async function (event) {
  ...
  try {
    const response = await axios.post(
      "http://65.0.185.140:3000/expense/add-expense",
      myObj,
...
async function deleteExpense(expenseid) {
  const response = await axios.delete(
    `http://65.0.185.140:3000/expense/delete-expense/${expenseid}`,
    { headers: { Authorization: token } }
  );
...
function getExpenses(page) {
  ...
  axios
    .get(
      `http://65.0.185.140:3000/expense/get-expense?page=${page}&itemPerPage=${ItemPerPage}`,
      { headers: { Authorization: token } }
    )
    .then((response) => {
...
function download() {
  axios
    .get("http://65.0.185.140:3000/expense/download", {
      headers: { Authorization: token },
    })
    .then((response) => {
...
function showUrlTable() {
  axios
    .get(`http://65.0.185.140:3000/expense/urlTable`, {
      headers: { Authorization: token },
    })
    .then((response) => {
      if (response.status === 201) {
...
async function showPremiumFeatures() {
  const response = await axios.get(
    `http://65.0.185.140:3000/premium/showLeaderBoard`,
    { headers: { Authorization: token } }
  );
...
document.getElementById("rzp-button1").onclick = async function (e) {
  const response = await axios.get(
    "http://65.0.185.140:3000/purchase/premiummembership",
    { headers: { Authorization: token } }
  );
  console.log(response);
  var options = {
    ...
    handler: async function (response) {
      await axios.post(
        "http://65.0.185.140:3000/purchase/updatetransactionstatusSuccess",
        {
  ...
  rzp1.on("payment.failed", async function (response) {
    console.log(response);
    await axios.post(
      "http://65.0.185.140:3000/purchase/updatetransactionstatusFail",
      {
        order_id: options.order_id,
      },
      { headers: { Authorization: token } }
